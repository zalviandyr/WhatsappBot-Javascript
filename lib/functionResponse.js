...
const functionResponse = {
    ...
    ig: async (client, message, args) => {
        if (args.length === 0) {
            ...
            fetcher(result.file)
                .then(async (dataUrl) => {
                    await client.sendFile(message.from, dataUrl.toString(), 'instagram.data', messageResponse.ig.success)
                })
                .catch(async () => {
            ...
            await insta.getStories(username).then(async (stories) => {
                if (stories.constructor === Array) {
                    if (position) {
                        ...
                        const url = stories[position - 1]
                        fetcherUrl(url)
                            .then(async (dataUrl) => {
                                await client.sendFile(message.from, dataUrl.toString(), 'igStories.data', messageResponse.igStories.success)
                            })
                            .catch(async () => {
                                await client.reply(message.from, messageResponse.igStories.error, message.id)
                            })
