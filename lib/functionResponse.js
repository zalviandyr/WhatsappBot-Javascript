...
const functionResponse = {
    ...
    lewd: async (client, message, args) => {
        if (args.length === 0) {
            ...
            fetch(url)
                ...
                .then(async buffer => {
                    const dataUrl = `data:image/png;base64,${buffer.toString('base64')}`
                    await client.reply(message.from, messageResponse.lewd.success, message.id)
                        .then(async () => {
                            await client.sendFile(sender, dataUrl, 'lewd.png', messageResponse.lewd.caption)
                        })
                })
                .catch(async () => {
    ...
    wallpaper: async (client, message) => {
        ...
        fetch(akaneko.nsfw.mobileWallpaper())
            ...
            .then(async buffer => {
                const dataUrl = `data:image/png;base64,${buffer.toString('base64')}`
                await client.reply(message.from, messageResponse.wallpaper.success, message.id)
                    .then(async () => {
                        await client.sendFile(sender, dataUrl, 'wallpaper.png', messageResponse.wallpaper.caption)
                    })
            })
            .catch(async () => {
    ...
    spam: async (client, message, args) => {
        if (args.length === 0) {
            ...
            if (option.includes(args[0])) {
                ...
                fs.readFile(`${config.path.json}/spam.json`, 'utf8', (_, data) => {
                    if (data) {
                    ...
                    fs.writeFile(`${config.path.json}/spam.json`, json, async (err) => {
                        if (err) {
