...
const {argExtractor, fetcher, checkFacebookLink, checkYoutubeLink} = require('./helper/utilities')
            ...
            argExtractor.getWord1Word2(args)
                .then(async (result) => {
                    ...
                    axios({
                        ...
                        await client.sendImage(message.from, response.data['result'], 'quotemaker.jpg', messageResponse.quoteMaker.success, message.id)
                    }).catch(async () => {
                        await client.reply(message.from, messageResponse.commonError, message.id)
                    })
                })
        ...
        if (args.length === 0) {
            ...
            axios({
                ...
                if (response.data['status_code'] !== 200) {
                    await client.sendFileFromUrl(message.from, response.data['url'], 'facebook.data', messageResponse.fbVideo.success, message.id)
                } else {
            ...
            axios({
                ...
                if (size[1] === 'KB' || (size[1] === 'MB' && size[0] * 10 < 100)) {
                    axios({
                        ...
                        await client.sendFile(message.from, dataUrl, `${title}.${ext}`, 'Neh master', message.id)
                    }).catch(async () => {
                        await client.reply(message.from, messageResponse.commonError, message.id)
                    })
                } else {
    ...
    ytSearch: async (client, message, args) => {
        if (args.length === 0) {
            ...
            axios({
                ...
                caption += `*${response.data['channel']}*\n`
                caption += '------------>>>>\n'
                caption += `${response.data['url']}\n`
                caption += '------------>>>>\n'
                caption += `${response.data['description']}`

                await client.sendFileFromUrl(message.from, response.data['thumbs'], 'thumb.png', caption, message.id)
            }).catch(async () => {
                await client.reply(message.from, messageResponse.commonError, message.id)
            })
        }
