...
const functionResponse = {
    about: async (client, message) => {
        fs.readFile(`${config.path.media}/about.jpg`, async (_, data) => {
            ...
            await client
                .reply(message.from, messageResponse.about, message.id)
                .then(async () => {
                    ...
                    fs.writeFile(
                        ...
                        async (err) => {
                            if (err) {
                                await client.reply(
                                    message.from,
                                    messageResponse.request.error,
                                    message.id,
                                )
                            } else {
                                await client.reply(
                                    message.from,
                                    messageResponse.request.success,
                                    message.id,
                                )
                            }
                        },
    ...
    sticker: async (client, message, args) => {
        if (message.type === 'chat') {
            ...
            await client
                .reply(message.from, messageResponse.sticker.wait, message.id)
                .then(async () => {
    ...
    jadwalShalat: async (client, message, args) => {
        if (args.length === 0) {
            await client.reply(
                message.from,
                messageResponse.jadwalShalat.wait,
                message.id,
            )
            fetch(
                ...
                .catch(async () => {
                    await client.reply(
                        message.from,
                        messageResponse.jadwalShalat.error,
                        message.id,
                    )
                })
        } else {
            await client.reply(
                message.from,
                messageResponse.jadwalShalat.waitSecondary,
                message.id,
            )
    ...
    quoteMaker: async (client, message, args) => {
        if (args.length === 0) {
            await client.reply(
                message.from,
                messageResponse.quoteMaker.help,
                message.id,
            )
        } else {
            await client.reply(
                message.from,
                messageResponse.quoteMaker.wait,
                message.id,
            )
            ...
            fetch(
                `https://terhambar.com/aw/qts/?kata=${quotes}&author=${author}&tipe=random`,
            )
                .then((res) => res.json())
                .then(async (json) => {
                    await client.sendImage(
                        ...
                        json.result,
                        'quotemaker.jpg',
                        messageResponse.quoteMaker.success,
                    )
                })
                .catch(async () => {
                    await client.reply(
                        message.from,
                        messageResponse.quoteMaker.error,
                        message.id,
                    )
                })
        }
    ...
    igStalk: async (client, message, args) => {
        if (args.length === 0) {
            await client.reply(
                message.from,
                messageResponse.igStalk.help,
                message.id,
            )
        } else {
            await client.reply(
                message.from,
                messageResponse.igStalk.wait,
                message.id,
            )
    ...
    ig: async (client, message, args) => {
        if (args.length === 0) {
            ...
            try {
                ...
                const result = await instagramDownload.downloadMedia(
                    url,
                    config.path.instagram,
                )
    ...
    igStories: async (client, message, args) => {
        if (args.length === 0) {
            await client.reply(
                message.from,
                messageResponse.igStories.help,
                message.id,
            )
        } else {
            await client.reply(
                message.from,
                messageResponse.igStories.wait,
                message.id,
            )
            const username = args[0]
                ...
                if (stories.constructor === Array) {
                    if (position) {
                        if (position > stories.length) {
                            ...
                            await client.reply(
                                message.from,
                                messageResponse.igStories.error,
                                message.id,
                            )
                            return
                        }
                        ...
                        await client.sendFile(
                            ...
                            dataUrl,
                            'igStories.data',
                            messageResponse.igStories.success,
                        )
                    } else {
                        await client.reply(
                            message.from,
                            `Ada ${stories.length} stories di instagram *${username}* master`,
                            message.id,
                        )
                    }
                } else {
    ...
    fbVideo: async (client, message, args) => {
        if (args.length === 0) {
            await client.reply(
                message.from,
                messageResponse.fbVideo.help,
                message.id,
            )
        } else {
            await client.reply(
                message.from,
                messageResponse.fbVideo.wait,
                message.id,
            )
            const url = args[0]
            fbVideos.low(url).then(async (result) => {
                ...
                if (result.url != null) {
                    await client.sendFileFromUrl(
                        ...
                        result.url,
                        'facebook.data',
                        messageResponse.fbVideo.success,
                    )
                } else {
                    await client.reply(
                        message.from,
                        messageResponse.fbVideo.error,
                        message.id,
                    )
                }
            })
    ...
    ytAudio: async (client, message, args) => {
        if (args.length === 0) {
            await client.reply(
                message.from,
                messageResponse.ytAudio.help,
                message.id,
            )
        } else {
            await client.reply(
                message.from,
                messageResponse.ytAudio.wait,
                message.id,
            )
            const url = args[0]
            ...
            ytAudio.on('info', (info, format) => {
                new Promise((resolve, reject) => {
                    ...
                    .then(() => {
                        ...
                        const path = `${config.path.youtube
                        ...
                        const file = ytAudio
                            ...
                            .addListener('finish', async () => {
                                ...
                                await client.sendFile(
                                    ...
                                    path,
                                    fileName,
                                    messageResponse.ytAudio.success,
                                )
                                file.close()
                            })
                    })
    ...
    ytVideo: async (client, message, args) => {
        if (args.length === 0) {
            await client.reply(
                message.from,
                messageResponse.ytVideo.help,
                message.id,
            )
        } else {
            await client.reply(
                message.from,
                messageResponse.ytVideo.wait,
                message.id,
            )
                        ...
                        const file = ytVideo
                            ...
                            .addListener('finish', async () => {
                                ...
                                await client.sendFile(
                                    ...
                                    path,
                                    fileName,
                                    messageResponse.ytVideo.success,
                                )
                                file.close()
                            })
                    })
                            ...
                            fs.writeFile(`${config.path.json}/tugas.json`, json,
                                async (err) => {
                                    if (err) {
                                        await client.reply(
                                            message.from,
                                            messageResponse.tugas.error,
                                            message.id,
                                        )
                                    } else {
                                        await client.reply(
                                            message.from,
                                            messageResponse.tugas.success,
                                            message.id,
                                        )
                                    }
                                })
    ...
    translate: async (client, message, args) => {
        if (args.length === 0) {
            ...
            await client.reply(message.from, messageResponse.translate.wait, message.id)
    ...
    waifu: async (client, message) => {
        ...
        await waifulabs.generateWaifus()
            .then((waifus) => {
                ...
                waifulabs.generateBigWaifu(waifu)
                    .then(async (waifu2) => {
                        const dataUrl = `data:image/png;base64,${waifu2.image}`
