            ...
            axios.get(`${stringValues.apiServer}/yt-audio?url=${url}`)
                .then(async (resultJson) => {
                    ...
                    if (size[1] === 'KB' || (size[1] === 'MB' && size[0] * 10 < 100)) {
                        await client.sendFileFromUrl(message.from, resultJson.data.url, `${title}.${ext}`, messageResponse.commonSuccess, message.id)
                    } else {
                ...
                .then(async (response) => {
                    ...
                    await client.sendFileFromUrl(message.from, response.data['thumbs'], 'thumb.png', caption, message.id)
                        .then(async () => {
                            ...
                            const urlAudio = `${stringValues.apiServer}/yt-audio?url=${response.data['url']}`
                            axios.get(urlAudio)
                                .then(async (resultJson) => {
                                    const size = resultJson.data.size.split(' ')
                                    // 10 mb
                                    if (size[1] === 'KB' || (size[1] === 'MB' && size[0] * 10 < 100)) {
                                        await client.sendFileFromUrl(message.from, resultJson.data.url, `audio.${resultJson.data['ext']}`, messageResponse.commonSuccess, message.id)
                                    } else {
                                        await client.reply(message.from, messageResponse.ytPlay.error, message.id)
                                    }
                                })
                                .catch(async (err) => {
                                    console.log(err)
                ...
                .then(async (members) => {
                    ...
                    for (let i = 0; i < julKelompok; i++) {
                        ...
                        for (let j = 0; j < anggotaKelompok; j++) {
                            ...
                            const person = (members[random]) ? members[random].pushname : '--*Kosong*--'
                            if (person) {
                                hasil.push(person)
                    ...
                    for (let i = 0; i < hasilKelompok.length; i++) {
                        ...
                        result += hasilKelompok[i].hasil.map((map) => `- ${map}`).join('\n')
                        result += '\n'
                    }

                    result += '\nOrang buangan:\n'
                    result += members.map((map) => `- ${map.pushname}`).join('\n')
