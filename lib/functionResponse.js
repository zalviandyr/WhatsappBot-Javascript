...
const {argExtractor, fetcher} = require('./helper/utilities')
...
const functionResponse = {
        ...
        if (args.length === 0) {
            ...
            axios({
                ...
                if (response.data['status_code'] !== 200) {
                    await client.sendFileFromUrl(message.from, response.data['url'], 'facebook.data', messageResponse.fbVideo.success,)
                } else {
    ...
    ytAudio: async (client, message, args) => {
        if (args.length === 0) {
            ...
            const url = args[0]
            axios({
                url: `${stringValues.apiServer}/yt-audio?url=${url}`
    ...
    ytVideo: async (client, message, args) => {
        if (args.length === 0) {
            ...
            axios({
                url: `${stringValues.apiServer}/yt-video?url=${url}`
            }).then(async (resultJson) => {
                const title = resultJson.data.title
                const ext = resultJson.data.ext
                const size = resultJson.data.size.split(' ')
    ...
    translate: async (client, message, args) => {
        if (args.length === 0) {
            ...
            const url = `${stringValues.apiServer}/translate?text=${word}`
            ...
            axios({
                ...
                await client.reply(message.from, messageResponse.translate.error, message.id)
            })
        }
    },
    ...
    waifu: async (client, message) => {
        ...
        const url = `${stringValues.apiServer}/anime-pic?genre=neko`
        axios({
            url
        }).then(async (response) => {
            await client.sendFileFromUrl(message.from, response.data['url'], 'waifu.png', messageResponse.waifu.success, message.id)
        }).catch(async () => {
    ...
    kabupatenKota: async (client, message, args) => {
        if (args.length === 0) {
            ...
            axios({
                ...
                await client.reply(message.from, messageResponse.kabupatenKota.error, message.id)
            })
        }
    },
    ...
    cuaca: async (client, message, args) => {
        if (args.length === 0) {
            ...
            const url = `${stringValues.apiServer}/cuaca?kabupaten=${kabupatenKota}&interval=sekarang`
    ...
    infoGempa: async (client, message) => {
        ...
        axios({
            ...
            await client.reply(message.from, messageResponse.infoGempa.error, message.id)
        })
    },
    ...
    bosan: async (client, message) => {
        ...
        axios({
            ...
            await client.reply(message.from, messageResponse.bosan.error, message.id)
        })
    },
// todo add commonError
    ...
    speech: async (client, message, args) => {
        if (args.length === 0) {
            ...
            const url = `${stringValues.apiServer}/speech?lang=${lang}&text=${word}`
            ...
            if (baseLang.includes(lang)) {
                axios({
                    ...
                    const dataUrl = `data:audio/ogg;base64,${buffer}`
        ...
        if (args.length === 0) {
            ...
            if (genreList.includes(genre)) {
                ...
                axios({
                    ...
                    await client.sendFile(message.from, response.data['url'], 'lewd.png', messageResponse.lewd.caption, message.id)
                }).catch(async () => {
    ...
    lewd2: async (client, message) => {
        await client.reply(message.from, messageResponse.lewd.wait, message.id)
        const url = `${stringValues.apiServer}/anime-pic?genre=random`

        axios({
            url
        }).then(async (response) => {
            await client.sendFile(message.from, response.data['url'], 'lewd2.png', messageResponse.lewd.caption, message.id)
        }).catch(async () => {
            await client.reply(message.from, messageResponse.lewd.error, message.id)
        })
    },
    ...
    wallpaper: async (client, message) => {
        ...
        axios({
            ...
            await client.sendFile(message.from, response.data['url'], 'wallpaper.png', messageResponse.lewd.caption, message.id)
        }).catch(async () => {
            await client.reply(message.from, messageResponse.lewd.error, message.id)
        })
    },
