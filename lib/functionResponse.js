...
const azlyrics = require('azlyrics-scraper')
...
const {filePath, argExtractor} = require('./helpers')
...
const functionResponse = {
    about: async (client, message) => {
        fs.readFile(filePath.media.about, async (_, data) => {
    ...
    ig: async (client, message, args) => {
        if (args.length === 0) {
            ...
            try {
                ...
                const result = await instagramDownload.downloadMedia(url, filePath.tempMedia.instagram)
            ...
            ytAudio.on('info', (info, format) => {
                new Promise((resolve, reject) => {
                    ...
                    .then(() => {
                        ...
                        const path = `${filePath.tempMedia.youtube}/${title}.${format.container}`
                        const file = ytAudio
    ...
    lewd: async (client, message, args) => {
        if (args.length === 0) {
            ...
            fetch(url)
                ...
                .then(async buffer => {
                    const dataUrl = `data:image/png;base64,${buffer.toString('base64')}`
                    await client.sendFile(message.from, dataUrl, 'lewd.png', messageResponse.lewd.caption, message.id)
                })
                .catch(async () => {
    ...
    lewd2: async (client, message) => {
        ...
        fetch(hentai.media)
            ...
            .then(async buffer => {
                const dataUrl = `data:image/png;base64,${buffer.toString('base64')}`
                await client.sendFile(message.from, dataUrl, 'lewd2.png', messageResponse.lewd.caption, message.id)
            })
            .catch(async () => {
                await client.reply(message.from, messageResponse.lewd.error, message.id)
            })
    ...
    wallpaper: async (client, message) => {
        ...
        fetch(akaneko.nsfw.mobileWallpaper())
            .then(res => res.buffer())
            .then(async buffer => {
                const dataUrl = `data:image/png;base64,${buffer.toString('base64')}`
                await client.sendFile(message.from, dataUrl, 'wallpaper.png', messageResponse.wallpaper.caption, message.id)
            })
            .catch(async () => {
    ...
    lirik: async (client, message, args) => {
        if (args.length === 0) {
            ...
            let result = `*Keyword:* ${keyword}\n`
            result += '---------------\n'

            azlyrics.getLyric(keyword)
                .then(async res => {
                    result += res.join('\n')
                    await client.reply(message.from, result, message.id)
                })
                .catch(async () => {
                    await client.reply(message.from, messageResponse.lirik.error, message.id)
                })
        }
    }
}
