...
const functionResponse = {
    ...
    inori: async (client, message) => {
        try {
            const url = `${stringValues.apiServer}/anime-pic?genre=inori`
            const response = await axios.get(url)
            await client.sendFileFromUrl(message.from, response.data['url'], 'inori.png', messageResponse.commonSuccess, message.id)
        } catch (err) {
            await client.reply(message.from, messageResponse.commonError, message.id)
            createErrorLog(message, err)
        }
    ...
    nekopoi: async (client, message) => {
        try {
            const url = `${stringValues.apiServer}/nekopoi`
            const {data} = await axios.get(url)
            let result = `*${data['title']}*\n\n`
            result += `*Movie ID:* ${data['movie_id']}\n`
            ...
            for (let i=0; i<data['downloads'].length; i++) {
                ...
                result += `*Title:* ${download['title']}\n`

                for (let j=0; j<download['download_link'].length; j++) {
                    const downloadLink = download['download_link'][j]
                    result += `*Server:* ${downloadLink['server']}\n`
                    result += `*Link:* ${downloadLink['link']}\n`
                }
