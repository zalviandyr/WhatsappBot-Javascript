...
const functionOwnerResponse = {
    ...
    stateOnAddedToGroup: async (client, message) => {
        const groupName = message.name
        ...
        if (stateList.includes(state)) {
            fs.readFile(filePath.json.state, 'utf8', (_, data) => {
                ...
                fs.writeFile(filePath.json.state, json, async (err) => {
                    if (err) {
                        await client.sendText(message.from, messageResponse.state.error)
                    } else {
                        let messageSuccess = messageResponse.state.success
                        messageSuccess += `\nInori join ke group *${groupName}*`
                        await client.sendText(message.from, messageSuccess)
                            .then(async () => {
                                const messageTemp = (state === stringValues.state.started) ? messageResponse.state.started : messageResponse.state.paused
                                await client.sendText(groupId.toString(), messageTemp)
                            })
