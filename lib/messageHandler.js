...
const messageHandler = async (client, message) => {
    ...
    if (command.startsWith('!')) {
        ...
        switch (command) {
            ...
            case 'tiktok': {
                await functionResponse.tiktok(client, message, args)
                break
            }
            case 'ytaudio': {
            ...
            case 'inori': {
                await functionResponse.inori(client, message)
                break
            }
            case 'husbu': {
                await functionResponse.husbu(client, message)
                break
            }
            case 'waifu': {
            ...
            case 'neonime': {
                await functionResponse.neonime(client, message, args)
                break
            }
            case 'artinama': {
            ...
            case 'admin': {
                checkIsGroupAdmin(client, message)
                    .then(async () => {
                        await functionResponse.admin(client, message, args)
                    })
                break
            }
            case 'adminall': {
                checkIsGroupAdmin(client, message)
                    .then(async () => {
                        await functionResponse.adminAll(client, message)
                    })
                break
            }
            case 'user': {
                checkIsGroupAdmin(client, message)
                    .then(async () => {
                        await functionResponse.user(client, message, args)
                    })
                break
            }
            case 'userall': {
                checkIsGroupAdmin(client, message)
                    .then(async () => {
                        await functionResponse.userAll(client, message)
                    })
                break
            }
            case 'add': {
            ...
            case 'nekopoi': {
                checkNsfw(client, message)
                    .then(async () => {
                        await functionResponse.nekopoi(client, message)
                    })
                break
            }
            case 'wallpaper': {
