...
const {checkIfOwner, checkIfGroupAdmin, checkSpam, checkNsfw} = require('./helper/authorization')
        ...
        switch (command) {
            case 'help': {
                await client.reply(message.from, messageResponse.help, message.id)
                    .then(async () => {
                        await client.reply(message.from, messageResponse.announcement, message.id)
                    })
                break
            }
            case 'tutorial': {
            ...
            case 'state': {
                checkIfOwner(client, message)
                    .then(async () => {
                        await functionOwnerResponse.state(client, message, args)
                    })
                break
            }
            case 'clean': {
                checkIfOwner(client, message)
                    .then(async () => {
                        await functionOwnerResponse.clean(client, message)
                    })
                break
            }
            case 'leavegroup': {
